...
#delete issue by range 시 numlist를 처리하기 위한 함수
...
#delete issue by range
@cli.command()
@click.option('--profile', default='default', help='profile name')
@click.option('--yes', '-y', is_flag=True, callback=abort_if_false, expose_value=False, prompt='Are you sure delete issue?')
@click.option('--range', 'range', callback=parse_num_list, help='delete issues by range')
@click.argument('project_id', type=str, required=True)
def delete_issue_range(profile, range, project_id):
    '''delete issues by range'''
    config = Config(profile, config_path)
    jira = Jira(config.url, config.username, config.password)
    try:
        for i in range.range:
            jira.delete_issue("{}-{}".format(config[project_id]))
